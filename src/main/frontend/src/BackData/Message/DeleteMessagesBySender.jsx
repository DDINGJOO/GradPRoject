...
const SentMessagesWithDelete = () => {
    const [messages, setMessages] = useState([]);
    const [error, setError] = useState(null);

    // 보낸 메시지 목록을 서버에서 가져오는 함수
    const fetchSentMessages = async () => {
        try {
            ...
            if (response.data.success) {
                setMessages(response.data.result.data); // 보낸 메시지 데이터를 저장
            } else {
    ...
    const deleteMessage = async (id) => {
        try {
            const response = await axios.delete(`http://localhost:8080/api/messages/sender/${id}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('accessToken')}`, // 로그인 시 받은 Access Token
                },
            });
...
export default SentMessagesWithDelete;
